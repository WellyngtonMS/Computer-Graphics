...
void LinearColorInterpolation(TPixel start, TPixel end, TPixel* source) {
    ...
    double aux = sqrt(pow(auxX, 2) + pow(auxY, 2)) / sqrt(pow(x, 2) + pow(y, 2));

    source->colors.R = end.colors.R + R * aux;
    source->colors.G = end.colors.G + G * aux;
    source->colors.B = end.colors.B + B * aux;
    source->colors.A = end.colors.A + A * aux;
}
